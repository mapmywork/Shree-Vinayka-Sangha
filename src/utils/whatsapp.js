...
export const generateWhatsAppLink = (message = '') => {
  ...
  return `https://wa.me/${CONTACT_INFO.phone.replace('+', '')}?text=${encodeURIComponent(textToEncode)}`;
};
...
export const generateFormWhatsAppLink = (data) => {
  ...
  return `https://wa.me/${CONTACT_INFO.phone.replace('+', '')}?text=${encodeURIComponent(message)}`;
};
